...
class Graph:
    ...
    def addData(self,correct,select):
        ...
        self.__correct = self.__answer==self.__solutaion[:self.__size_ans]
        ...
        try:
            ...
            self.__new_c_data[self.__size_sol] = 2/(1+self.__correct/self.__size_sol)
            self.__new_r_data = {i:j for i,j in list(zip(self.__n_card,list(np.zeros(20))))}
            self.__new_r_data[self.__size_sol] = np.sqrt(3*np.pi*self.__size_ans/2)
    ...
    def save_graph(self):
        ...
        self.__x_value = [np.sqrt(k*(2*i-1))/j for i,j,k in list(zip(self.__avg_c,self.__avg_r,self.__n_card))]

        plt.plot(self.__n_card,self.__x_value,'bo')
